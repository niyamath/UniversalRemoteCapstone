...
void loop() {
  ...
  // Initializing Led sequence coresseponding to buttons.
  ledProgram.drive();
  ledButton1.drive();
  ledButton2.drive();
  ledButton3.drive();
...
// Here the statement irrecv.decode(&results) works like an interrupt to record IR signals to the receiver
// and the purpose of this code block in the if statement is to display the recording via the status led 
...
if(digitalRead(BUTTON_THREE)==HIGH){
      //transfer sequence: write this to EEPROM cell
      Serial.println("IN state transfer  loop");
      EEPROMstore(THREE);
      delay(500);
      EEPROMfetch(THREE);
...
break;
  ...
  //The use of this function is to assign and display working of each button respective to their LEDs
  // If button pressed, send the code.
  void playBack(){
    ...
    int lastButtonState1=buttonOne;
    int lastButtonState2=buttonTwo;
    int lastButtonState3=buttonThree;
    int lastButtonState4=buttonFour;

    // Please note if the sendCode() was fired the IR receiver was disabled.
    //The solution was to bring the Renable the receiver again if the particular button was Released.
